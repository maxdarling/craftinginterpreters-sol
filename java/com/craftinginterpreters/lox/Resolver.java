...
// idx     : position of variable's declaration in its scope. used for an optimization
//           (fast runtime lookups via array indexing rather than var name in map).
//
// note: this is a little tricky. but you should be able to picture (distance, idx) being sufficient to resolve a
// variable access to it's definition. as long as both resolver and interpreter (via environment) define variables
// in sequential order, the indices will match up.
...
class Resolver implements Expr.Visitor<Void>, Stmt.Visitor<Void> {
  ...
  private static class Variable {
    final Token name;
    State state;
    int slot; // unique identifier for var (in declaration scope)
